...
func (tx *Tx) init() {
	tx.AccountPlatform = NewAccountPlatformClient(tx.config)
	tx.AccountUser = NewAccountUserClient(tx.config)
	tx.AuthItem = NewAuthItemClient(tx.config)
	tx.AuthMs = NewAuthMsClient(tx.config)
	tx.AuthMsMenu = NewAuthMsMenuClient(tx.config)
	tx.AuthRole = NewAuthRoleClient(tx.config)
	tx.AuthRoleStaff = NewAuthRoleStaffClient(tx.config)
	tx.ProductAttr = NewProductAttrClient(tx.config)
	tx.ProductAttrValue = NewProductAttrValueClient(tx.config)
	tx.ProductBrands = NewProductBrandsClient(tx.config)
	tx.ProductCategory = NewProductCategoryClient(tx.config)
	tx.ProductSku = NewProductSkuClient(tx.config)
	tx.ProductSkuStock = NewProductSkuStockClient(tx.config)
	tx.ProductSpu = NewProductSpuClient(tx.config)
	tx.ProductSpuSkuAttrMap = NewProductSpuSkuAttrMapClient(tx.config)
	tx.SkrMember = NewSkrMemberClient(tx.config)
	tx.StaffInfo = NewStaffInfoClient(tx.config)
}
...
// applies a query, for example: AccountPlatform.QueryXXX(), the query will be executed
